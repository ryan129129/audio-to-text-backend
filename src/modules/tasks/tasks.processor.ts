...
export class TasksProcessor extends WorkerHost {
  ...
  async process(job: Job<TaskJobData>): Promise<void> {
    const { task_id, source_type, source_url, params } = job.data;
    this.logger.log(`Processing task ${task_id}`);

    try {
      // 更新状态为 processing
      await this.tasksService.updateTaskStatus(task_id, TaskStatus.PROCESSING);
      ...
      await this.tasksService.updateTaskStatus(task_id, TaskStatus.SUCCEEDED, {
      ...
      if (task && !task.is_trial && task.user_id) {
        // 付费任务需要扣费
        // 这里简化处理，实际应该用 Webhook 统一扣费
      }

      this.logger.log(`Task ${task_id} completed successfully`);
    } catch (error) {
      this.logger.error(`Task ${task_id} failed: ${error}`);
      await this.tasksService.updateTaskStatus(task_id, TaskStatus.FAILED, {
